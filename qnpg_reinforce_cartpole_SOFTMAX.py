...
def compute_qfim(s,w):
                                ...
                                if filename_save in ["qfim", "qfim_block_diag", "fim"]:
                                    fim += 0.1 * np.eye(len(w))
...
if __name__ == '__main__':
    ...
    np.save("cartpole_{}_nNG_ - {} || {}.npy".format(filename_save,ng,str(processid)), rewards_q)
    np.save("cartpole_{}_nNG_grads_norm - {} || {}.npy".format(filename_save,ng,str(processid)), grads_q)
    np.save("cartpole_{}_nNG_vars - {} || {}.npy".format(filename_save,ng,str(processid)), vars)
