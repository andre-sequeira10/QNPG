...
@qml.qnode(device_fisher,interface="torch",diff_method="backprop")#, parallel=True)#, interface="autograd")
...
class policy_estimator_q(nn.Module):        
    ...
    def forward(self, state):
        ...
        if policy == "Q":
            ...
            #action_probs = F.softmax(beta*out, dim=-1)

            '''
            partitions = list(map(len, np.array_split(list(range(2**n_qubits)),3)))
            indexes = torch.split(out, partitions)
            action_probs = torch.stack([torch.sum(i) for i in indexes])
            action_probs /= torch.sum(action_probs)
            '''
            action_probs = torch.zeros(3)
            
            for i in range(2**n_qubits):

                a = i % 3
                action_probs[a] += out[i]
            #out = self.qlayer(torch.FloatTensor(state))
...
def compute_qfim(s,w):
                                ...
                                if filename_save == "qfim_4":
                                    mt_fn = qml.metric_tensor(qcircuit_fisher, approx=None, aux_wire=n_qubits)#,hybrid=True)
                                    fim = mt_fn(np.array(s,requires_grad=False), np.array(w, requires_grad=True))#[1]
                                elif filename_save == "qfim":
                                    mt_fn = qml.metric_tensor(qcircuit_fisher, approx=None, aux_wire=n_qubits)#,hybrid=True)
                                    fim = 4*mt_fn(np.array(s,requires_grad=False), np.array(w, requires_grad=True))#[1]
                                elif filename_save == "qfim_block_diag":
                                    mt_fn = qml.metric_tensor(qcircuit_fisher, approx="block-diag", aux_wire=None)#,hybrid=True)
                                    fim = 4*mt_fn(np.array(s,requires_grad=False), np.array(w, requires_grad=True))#[1]
                                elif filename_save == "fim":
                                    fim = qml.qinfo.classical_fisher(qcircuit_fisher)(np.array(s,requires_grad=False), np.array(w, requires_grad=True))
                                elif filename_save == "qfim_half":
                                    mt_fn = qml.metric_tensor(qcircuit_fisher, approx=None, aux_wire=n_qubits)
                                    fim = 4 * mt_fn(np.array(s, requires_grad=False), np.array(w, requires_grad=True))
                                    
                                    fim += 0.1 * np.eye(len(w))
                                    ...
                                    fim = np.dot(eigenvectors, np.dot(Lambda_sqrt, eigenvectors.T))
                                ...
                                if filename_save in ["qfim","qfim_4", "qfim_block_diag", "fim"]:
                                    fim += 0.1 * np.eye(len(w))
...
class LinearAnnealing:
    def __init__(self, start_value, end_value, n_steps):
    ...
    def step(self):
        if self.current_step < self.n_steps:
            beta = self.start_value + self.current_step * (self.end_value - self.start_value) / self.n_steps
            self.current_step += 1
            return beta
        else:
...
def reinforce(env, policy_estimator, num_episodes=600,
   ...
   #for name,param in policy_estimator.named_parameters():
    ...
    annealer = LinearAnnealing(start_temp, end_temp, total_steps)
    ...
    for ep in range(num_episodes):
        ...
        while complete == False:
            ...
            #s_0 = normalize(s_0)
            newvalue2= ((s_0[5] + 9*np.pi)/(18*np.pi)) * (2*np.pi) - np.pi
            newvalue1= ((s_0[4] + 4*np.pi)/(8*np.pi)) * (2*np.pi) - np.pi
            theta1 = np.arccos(s_0[0])
            theta2 = np.arccos(s_0[2])
            s_0 = np.array([theta1,theta2,newvalue1,newvalue2])

            #action, action_log_prob = policy_estimator.forward(s_0,beta)
...
if __name__ == '__main__':
    ...
    np.save("acrobot_{}_10_NG_ - {} || {}.npy".format(filename_save,ng,str(processid)), rewards_q)
    np.save("acrobot_{}_10_NG_grads_norm - {} || {}.npy".format(filename_save,ng,str(processid)), grads_q)
    np.save("acrobot_{}_10_NG_vars - {} || {}.npy".format(filename_save,ng,str(processid)), vars)
